 ...
 * <p>
 * This allows you to subclass <code>User</code> and provide an additional bean getter for a salt.
 * You should use a synthetic value that does not change, such as a database primary key. Do not use
 * <code>username</code> if it is likely to change.
 *
 * @author Ben Alex
 */
public class ReflectionSaltSource implements SaltSource, InitializingBean {
   ...
   * <p>
   * The property identified by <code>userPropertyToUse</code> must be available from the passed
   * <code>User</code> object. If it is not available, an {@link AuthenticationServiceException}
   * will be thrown.
   *
   * @param user which contains the method identified by <code>userPropertyToUse</code>
   *
   * @return the result of invoking <tt>user.userPropertyToUse()</tt>, or if the method doesn't
   *         exist, <tt>user.getUserPropertyToUse()</tt>.
   *
  ...
  private Method findSaltMethod(UserDetails user) {
    ...
    if (saltMethod == null) {
      ...
      if (saltMethod == null) {
        throw new AuthenticationServiceException("Unable to find salt method on user Object. Does the class '" + user.getClass().getName()
  ...
  /**
   * The method name to call to obtain the salt. Can be either a method name or a bean property
   * name. If your <code>UserDetails</code> contains a <code>UserDetails.getSalt()</code> method,
   * you should set this property to "getSalt" or "salt".
   *
